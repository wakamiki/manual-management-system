...
| MAN-004A | 正常 | changeNote 未入力 | 新規作成は問題なく保存できる |
...
| MAN-013G | 正常 | status 初期条件 | 初期表示は `PENDING` / `APPROVED` のみ対象 |
| MAN-013H | 正常 | keyword 値保持 | 検索後に入力値がフォームへ再表示される |
| MAN-013I | 正常 | categoryIds 値保持 | 検索後にカテゴリチェック状態が保持される |
| MAN-013J | 正常 | statuses 値保持 | 検索後にステータスチェック状態が保持される |
| MAN-013K | 正常 | アコーディオンID一意 | 複数行で各行が個別に開閉できる |
| MAN-013L | 異常 | 表示データnull | Model 未設定時に EL エラーを検知できる |
