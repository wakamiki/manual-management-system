...
| FORM-009 | 正常 | approve 押下時の確認モーダル表示 | 「そのまま承認 / 追加する」を選択できる |
| FORM-010 | 正常 | approveモーダル「追加する」選択 | インライン入力パネルが開き、承認URLへ submit される |
| FORM-011 | 正常 | approveモーダル「そのまま承認」選択 | changeNote 空で承認処理が実行される |
| FORM-012 | 異常 | manual-detail の承認/差し戻し/アーカイブ/復帰で例外発生 | index ではなく manual-detail に戻りエラー表示される |
...
### 9-2A. カテゴリ重複モーダル / 同名処理（2026-04-28）
| No | 種別 | テスト観点 | 期待結果 |
| --- | --- | --- | --- |
| CAT-010 | 異常 | 同名の使用中カテゴリが存在する状態で作成/更新 | モーダルは出ず、エラーメッセージ表示で保存されない |
| CAT-011 | 正常 | 同名の停止中カテゴリが存在する状態で作成/更新 | 確認モーダルが表示される |
| CAT-012 | 正常 | 同名停止中カテゴリモーダルでキャンセル | 保存されず、画面状態を維持する |
| CAT-013 | 正常 | 同名停止中カテゴリモーダルで続行 | 旧カテゴリ配下マニュアルが ARCHIVED になり、処理が完了する |
| CAT-014 | 正常 | モーダル続行時の送信先制御（CREATE/EDIT） | CREATE は `/categories/create`、EDIT は `/categories/{id}/update` に送信される |
| CAT-015 | 異常 | カテゴリ管理の新規/更新で例外発生 | index ではなく category-management に戻りエラー表示される |
...
| ADM-012 ✅ | 正常 | index 自動リロードフラグ | DB更新後に list 画面へ戻った際、更新反映のため自動リロードが1回だけ動作する |
