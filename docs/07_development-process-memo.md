...
### 作業概要
業務マニュアル管理システムの設計資料が完成したため、提出時に参照しやすいようにREADMEとdocsの導線を整理した。  
あわせて、公開資料に機密情報や非公開SQLへの参照が残らないように見直した。

### 実施内容
- READMEの資料一覧を概要直下へ配置し、実ファイル名に合わせてリンクを修正
  - `docs/portfolio/01_system-overview.pdf`
  - `docs/portfolio/02_operation-guide.pdf`
- ポートフォリオ資料の読み方を追加
  - `docs/portfolio/README.md`
- 公開docsから、非公開管理の初期データSQLを直接参照する記述を削除
- DB移行情報を公開可能な内容へ整理
  - `V1__create_tables.sql` は公開
  - 初期データSQLは機密情報として別管理
- セキュリティ確認資料を追加
  - 認証/認可
  - SQLインジェクション対策
  ...
  - H2コンソール本番無効化
  - セキュリティヘッダ
- release-noteは単独資料にせず、必要な内容はREADME/開発記録/セキュリティ確認資料へ集約する方針に整理
